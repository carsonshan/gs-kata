...
public class Exercise3Test extends CompanyDomainForKata
{
     ...
     */
    private static final Predicate<Customer> CUSTOMER_FROM_LONDON = Predicates.attributeEqual(Customer.TO_CITY, CITY_LONDON);
    ...
    {
        boolean anyCustomersFromLondon = getCompanyCustomers().anySatisfy(CUSTOMER_FROM_LONDON);
    ...
    {
        boolean allCustomersFromLondon = getCompanyCustomers().allSatisfy(CUSTOMER_FROM_LONDON);
    ...
    {
        int numberOfCustomerFromLondon = getCompanyCustomers().count(CUSTOMER_FROM_LONDON);
    ...
    {
        MutableList<Customer> customersFromLondon = getCompanyCustomers().select(CUSTOMER_FROM_LONDON);
    ...
    {
        MutableList<Customer> customersNotFromLondon = getCompanyCustomers().reject(CUSTOMER_FROM_LONDON);
    ...
    {
        Customer mary = this.company.getCustomerNamed(CUSTOMER_MARY);
        Assert.assertEquals("customer's name should be Mary", CUSTOMER_MARY, mary.getName());
    }
