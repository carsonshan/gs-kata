...
public class Exercise7Test extends CompanyDomainForKata {
    ...
    public void sortedTotalOrderValue() {
        MutableList<Double> sortedTotalValues = getCompanyCustomers().collect(Customer.TO_TOTAL_ORDER_VALUE).toSortedList();
    ...
    public void maximumTotalOrderValue() {
        Double maximumTotalOrderValue = getCompanyCustomers().collect(Customer.TO_TOTAL_ORDER_VALUE).max();
    ...
    public void customerWithMaxTotalOrderValue() {
        Customer customerWithMaxTotalOrderValue = getCompanyCustomers().maxBy(Customer.TO_TOTAL_ORDER_VALUE);
    ...
    public void supplierNamesAsTildeDelimitedString() {
        String tildeSeparatedNames = FastList.newListWith(getCompanySuppliers()).collect(Supplier.TO_NAME).makeString("~");
        Assert.assertEquals(
    ...
    public void deliverOrdersToLondon() {
        getCompanyCustomers().select(Predicates.attributeEqual(Customer.TO_CITY, CITY_LONDON)).flatCollect(Customer.TO_ORDERS).forEach(Order.DELIVER);
