...
public class Exercise4Test extends CompanyDomainForKata {
    ...
    public void improveGetOrders() {
        Verify.assertSize(5, getCompanyOrders());
    }
    ...
    public void findItemNames() {
        MutableSet<String> actualItemNames = getCompanyOrders()
    ...
    public void findCustomerNames() {
        MutableList<String> names = getCompanyCustomers().collect(Customer.TO_NAME);

        MutableList<String> expectedNames = FastList.newListWith(CUSTOMER_FRED, CUSTOMER_MARY, CUSTOMER_BILL);
